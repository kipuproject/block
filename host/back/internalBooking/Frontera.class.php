...
class FronterainternalBooking{
    ...
    if (is_array($dataUser)) {
      $response->name = $dataUser[0]["NOMBRE"] . " " . $dataUser[0]["APELLIDO"];
      $response->phone = $dataUser[0]["PHONE"];
      $response->email = $dataUser[0]["EMAIL"];
      $response->birthdate = $dataUser[0]["BIRTHDAY"];
      $response->country = $dataUser[0]["COUNTRY"];
